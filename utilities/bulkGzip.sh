#!/bin/bash
#SBATCH -J zipItGood
#SBATCH -o /hpcfs/users/%u/log/gzip.slurm-%j.out
...
usage()
{
echo "# bulkGzip.sh save space by compressing all the things.
#
# Usage: 
...
readarray -t fileToCompress < $inputFile

# When a big file comes along, you must zip it
gzip ${fileToCompress[$SLURM_ARRAY_TASK_ID]}
