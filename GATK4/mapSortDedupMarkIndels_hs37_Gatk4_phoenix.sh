...
scriptBase=/hpcfs/groups/phoenix-hpc-neurogenetics/scripts/git/mark/map-n-call
...
if [ -z "$config" ]; then # If no config file specified use the default
   config=$scriptBase/configs/BWA-GATKHC.GRCh37_full_analysis_set_phoenix.cfg
fi
source $config
if [ ! -z "$inputFile" ]; then
...
tmpDir=/hpcfs/groups/phoenix-hpc-neurogenetics/tmp/BWA/${outPrefix[$SLURM_ARRAY_TASK_ID]} # Use a tmp directory for all of the GATK and samtools temp files
if [ ! -d "$tmpDir" ]; then
	mkdir -p $tmpDir
fi
if [ ! -d "$workDir" ]; then
