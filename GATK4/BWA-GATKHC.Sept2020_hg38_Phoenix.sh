...
GATKHCjob=`sbatch $scriptDir/GATK.HC_hg38_phoenix.sh -c $CONFIG -p $OUTPREFIX -S $SAMPLE -o $WORKDIR`
GATKHCjob=$(echo $GATKHCjob | cut -d" " -f4)
sbatch --export=ALL --dependency=afterok:${MergeJob} $scriptDir/PicardCollectWGSMetrics_hg38_phoenix.sh -c $CONFIG -p $OUTPREFIX -S $SAMPLE -o $WORKDIR
