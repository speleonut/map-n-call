...
if [ -z "$config" ]; then # If no config file specified use the default
   config=/hpcfs/users/a1149120/Scripts/Gatk_hg38/BWA-GATKHC.GRCh38_full_analysis_set_phoenix.cfg
fi
source $config
...
if [ -z "$inputDir" ]; then #If no input directory specified then use default
	inputDir=/hpcfs/users/${USER}/gVCF/gvcfs
...
java -Xmx20g -Djava.io.tmpdir=$tmpDir/$bed -jar $GATKPATH/GenomeAnalysisTK.jar GenomicsDBImport \
$(cat inputGVCF.txt) \
--genomicsdb-workspace-path /fast/users/a1149120/gVCF/gvcfs/ExomeTest_database \
--intervals /data/neurogenetics/git/PhoenixScripts/shared/scripts/hg38_interval.list.bed

java -Xmx20g -Djava.io.tmpdir=$tmpDir/$bed -jar $GATKPATH/GenomeAnalysisTK.jar GenotypeGVCFs \
-R $GATKREFPATH/$GATKINDEX \
-V gendb://ExomeTest_database \
-O ExomeTest_output.vcf
...
-R $GATKREFPATH/$GATKINDEX \
-V /fast/users/a1149120/gVCF/gvcfs/ExomeTest_output.vcf \
