...
#tmpDir=/hpcfs/groups/phoenix-hpc-sacgf/scratch/ali/tmp
...
if [ ! -d "${tmpDir}" ]; then mkdir -p "${tmpDir}" || { echo "ERROR: Failed to create directory ${tmpDir}"; exit 1; }
fi

# Check if tmpDir is accessible
if [ ! -r "${tmpDir}" ] || [ ! -w "${tmpDir}" ]; then echo "ERROR: ${tmpDir} is not accessible. Please check permissions." 
exit 1
fi
